...
bool captureFrameWithLibcamera(cv::Mat& frame)
{
    ...
    std::string cmd = std::string("/usr/bin/libcamera-still -n -t 1000 -o ") + path + " >" + logPath + " 2>&1";
    ...
    if (result != 0) 
    {
        ...
        std::cerr << "[Camera] libcamera-still nie powiodlo sie (kod " << exitCode
                  << "). Szczegoly w " << logPath << std::endl;
        return false;
    }
    ...
    if (frame.empty()) 
    {
        std::cerr << "[Camera] Nie udalo sie wczytac obrazu z " << path << std::endl;
        return false;
    }
    return true;
}
...
float Camera::estimateGrassHeightCm()
{
    ...
    if (heightCm < 0.0f) 
    {
        std::cerr << "[Camera] Nie wykryto trawy w kadrze" << std::endl;
        return -1.0f;
    }
