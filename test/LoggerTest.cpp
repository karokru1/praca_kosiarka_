...
TEST(LoggerTest, initFailsOnReadOnlyDatabase)
{
    ...
    std::filesystem::permissions(dbPath, std::filesystem::perms::owner_read, std::filesystem::perm_options::replace);
    Logger logger(dbPath);

    EXPECT_FALSE(logger.init());

    std::filesystem::permissions(dbPath, std::filesystem::perms::owner_all, std::filesystem::perm_options::replace);
    std::filesystem::remove(dbPath);
}
...
TEST(LoggerTest, logReadingFailsWhenTableMissing)
{
    ...
    EXPECT_EQ(sqlite3_open(dbPath.c_str(), &db), SQLITE_OK);
    EXPECT_EQ(sqlite3_exec(db, "DROP TABLE sensor_logs;", nullptr, nullptr, nullptr), SQLITE_OK);
...
TEST(LoggerTest, logReadingFailsWhenConstraintViolated)
{
    ...
    const char* createSql =
        ...
        ");";
    EXPECT_EQ(sqlite3_exec(db, createSql, nullptr, nullptr, nullptr), SQLITE_OK);
